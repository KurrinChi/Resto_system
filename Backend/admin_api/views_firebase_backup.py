...
def profile(request):
    ...
    try:
        ...
        if request.method == 'GET':
            # Try to get admin01 (primary admin) directly first for speed
            from utils.firebase_config import db
            admin_doc = db.collection(COLLECTIONS['users']).document('admin01').get()
            
            if admin_doc.exists:
                admin = {'id': admin_doc.id, **admin_doc.to_dict()}
                admins = [admin]
            else:
                # Fallback to query if admin01 doesn't exist
