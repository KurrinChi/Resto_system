...
export const exportReportToCSV = (stats, revenueData, ordersStatusData, topItemsData, customerGrowthData, categorySales, reportType, dateRange) => {
...
export const exportSingleSheetReport = (stats, revenueData, ordersStatusData, topItemsData, customerGrowthData, categorySales, reportType, dateRange) => {
    // Create comprehensive CSV with all data
    let csvContent = "";
    // Add title
    csvContent += `${reportType.toUpperCase()} REPORT\n`;
    csvContent += `Generated: ${new Date().toLocaleString()}\n`;
    csvContent += `Date Range: ${dateRange.start || "All"} to ${dateRange.end || "All"}\n\n`;
    // Summary Statistics
    csvContent += "SUMMARY STATISTICS\n";
    csvContent += "Metric,Value,Growth\n";
    csvContent += `Total Revenue,$${stats.totalRevenue.toFixed(2)},${stats.revenueGrowth}%\n`;
    ...
    categorySales.forEach((item) => {
        csvContent += `"${item.category}",${item.orders},$${item.revenue.toFixed(2)},${item.growth}%\n`;
    });
    // Create and download
    const blob = new Blob([csvContent], { type: "text/csv;charset=utf-8;" });
    const link = document.createElement("a");
    ...
    link.setAttribute("href", url);
    link.setAttribute("download", `${reportType}_comprehensive_report_${timestamp}.csv`);
    link.style.visibility = "hidden";
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
