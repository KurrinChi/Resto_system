...
// Provide a minimal module declaration so TypeScript won't complain about dynamic import
...
const PH_BOUNDS = L.latLngBounds(
  L.latLng(4.215806, 116.928055), // Southwest
    ...
    <div className="relative z-[100]">
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 px-4 py-2 text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-[#2b211f] text-white placeholder-gray-400 border border-gray-600"
        />
        <button
      ...
      {/* Search Results Dropdown rendered in a portal above all layers */}
      {searchResults.length > 0 && dropdownPos && createPortal(
        <div
          className="border rounded-lg shadow-2xl max-h-60 overflow-y-auto bg-[#2b211f] border-gray-600"
          style={{ position: 'fixed', left: dropdownPos.left, top: dropdownPos.top + 4, width: dropdownPos.width, zIndex: 100000 }}
        >
          {searchResults.map((result, index) => (
            <button
              key={index}
              onClick={() => handleSelect(result)}
              className="w-full text-left px-4 py-3 hover:bg-[#3a2c29] transition border-b border-gray-700 last:border-b-0"
            >
...
interface MapProps {
  ...
  lng?: number;
  zoom?: number;
  points?: Point[]; // additional points to render on the map
  onLocationSelect?: () => void; // callback when location is confirmed
}

const Map = forwardRef<{ handleSelectSearchResult: (result: any) => void }, MapProps>(({
  lat = 14.871852789906772,
  lng = 120.79893918035896,
  zoom = 15,
  points = [],
  onLocationSelect,
}, ref) => {
  ...
  useEffect(() => {
    ...
    if (points && points.length) {
      extraMarkersRef.current = points.map((p) =>
  ...
  useEffect(() => {
    ...
    const pts: Point[] = points || [];
    extraMarkersRef.current = pts.map((p) =>
      L.marker([p.lat, p.lng], {
        icon: L.icon({
          iconUrl: "https://cdn-icons-png.flaticon.com/512/684/684908.png",
          iconSize: [28, 28],
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div
        ...
        style={{ height: "60vh" }} // map height is 60% of the viewport height
      />

      {/* Controls - Below the map */}
      <div className="flex flex-wrap gap-2 justify-center items-center pt-2 border-t border-gray-200">
        {["Add Marker", "Clear Marker"].map((label, i) => {
          const handler = i === 0 ? handleAddMarker : handleClearMarker;
          return (
            <button
              key={i}
              onClick={handler}
              className="px-3 py-1.5 text-sm bg-[#212121] text-white rounded-lg shadow hover:bg-gray-800 transition"
            >
              {label}
            </button>
          );
        ...
        <button
          ...
          className="px-4 py-1.5 text-sm bg-green-600 text-white rounded-lg shadow hover:bg-green-700 transition disabled:opacity-50 disabled:cursor-not-allowed font-semibold"
        >
          Confirm Location
        </button>
      </div>
