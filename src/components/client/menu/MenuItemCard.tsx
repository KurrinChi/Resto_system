...
export const MenuItemCard: React.FC<MenuItemCardProps> = ({ item, onAddToOrder }) => {
  const handleAdd = () => {
    if (onAddToOrder) {
    ...
    <Card padding="md" className="flex flex-col justify-between" style={{ backgroundColor: THEME.colors.background.tertiary, borderColor: THEME.colors.border.DEFAULT, color: THEME.colors.text.primary }}>
      <div>
        <div className="h-36 rounded-lg mb-3 relative overflow-hidden">
          ...
          {item.isNewOffer && (
            <span className="absolute top-2 right-2 px-2 py-1 rounded text-xs font-semibold bg-green-600 text-white">
              New
            </span>
          )}
        </div>
